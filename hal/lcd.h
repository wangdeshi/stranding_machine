...
#define lcd_show_digit(addr, n, digit, align, show_digit_bits, clear_enable, clear_bit) do {        \
    _lcd_addr = (addr);                                                                             \
    _lcd_n = (n);                                                                                   \
    _lcd_digit = (digit);                                                                           \
    _lcd_align = (align);                                                                           \
...
#define lcd_show_digit_clear_bit(addr, n, digit, x, clear_bit) lcd_show_digit((addr), (n), (digit), LCD_SHOW_DIGIT_ALIGN_RIGHT, (x), 1, (clear_bit))
